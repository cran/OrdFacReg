    ...
    display = 0, eps = 0) 
{
    ...
    q <- dim(V)[2]
    if (type == "LS") {
        lik.value <- function(beta, D, Y) {
            return(lmSS(beta, D, Y)$L)
        }
        beta.fct <- function(D, Y, A, JJs, q) {
            return(LSEsubspace(D, Y, A, JJs, q)$beta)
        }
        dL.fct <- function(beta, D, Y) {
            return(lmSS(beta, D, Y)$dL)
        }
    }
    if (type == "logreg") {
        lik.value <- function(beta, D, Y) {
            return(logRegLoglik(beta, D, Y)$L)
        }
        beta.fct <- function(D, Y, A, JJs, q) {
            return(logRegSubspace(D, Y, A, JJs, q)$beta)
        }
        dL.fct <- function(beta, D, Y) {
            return(logRegDeriv(beta, D, Y)$dL)
        }
    }
    A <- 1:q
    ...
    if (length(A) > 0) {
        while (max(crit[A]) > eps) {
            ...
            crit <- t(B) %*% dL.fct(beta, D, Y)
            if (display == 1) {
                print(paste("iter1 = ", iter1, " / L = ", round(lik.value(beta, 
                  D, Y), 3), " / A = ", list(A), sep = ""))
            }
            if (length(A) == 0) {
                break
