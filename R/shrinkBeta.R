...
function (Y, A, JJs, q) 
{
    ...
    for (u in 1:length(JJs)) {
        ...
        if (length(JJ) > 1) {
            for (v in 1:(length(JJ) - 1)) {
                JJs.exp <- rbind(JJs.exp, c(u, JJ[v:(v + 1)]))
            }
        }
    }
        ...
        if (nactive >= 1) {
            ...
            if (ndummy > 1) {
                if (is.na(tmp[1, 2]) == FALSE) {
                  ...
                  if (nactive >= 2) {
                    for (i in 2:length(tmp[, 1])) {
                      if (tmp[i, 2] == sum1[length(sum1[, 1]), 
                        3]) {
                        sum1[length(sum1[, 1]), 3] <- tmp[i, 
                          3]
                      }
                      else {
                        sum1 <- rbind(sum1, tmp[i, ])
                      }
                    }
                  }
                  sums <- rbind(sums, sum1)
                }
                if ((JJs.A.u[1, 5] == 1) && (JJs.A.u[2, 5] == 
                  0)) {
                  ...
                  if ((ndummy >= 3) && (nactive >= 2)) {
                    ...
                    if (nactive >= 3) {
                      for (i in 3:length(tmp[, 1])) {
                        if (tmp[i, 2] == sum2[length(sum2[, 1]), 
                          3]) {
                          sum2[length(sum2[, 1]), 3] <- tmp[i, 
                            3]
                        }
                        else {
                          sum2 <- rbind(sum2, tmp[i, ])
                        }
                      }
                    }
                ...
                if ((JJs.A.u[1, 5] == 1) && (JJs.A.u[2, 5] == 
                  ...
                  rem3 <- NULL
                  sum3 <- NULL
                  tmp3 <- cbind(tmp, tmp[1, 3] + (1:length(tmp[, 
                    1])) - 1)
                  rem3 <- cbind(rem3, tmp3[tmp3[, 3] == tmp3[, 
                    4], -4])
                  max.rem <- max(rem3[, 3])
                  max.q <- max(JJs.A[, 4])
                  tmp3.sum <- matrix(tmp3[tmp3[, 3] != tmp3[, 
                    4], -4], ncol = 3)
                  if ((nrow(tmp3.sum) > 0) && (max.q > max.rem) && 
                    (sum(JJs.A[JJs.A[, 3] > max.rem, 5], na.rm = TRUE) > 
                      0)) {
                    sum3 <- matrix(tmp3.sum[1, ], ncol = 3)
                    if (nrow(tmp3.sum) > 1) {
                      for (i in 2:length(tmp3.sum[, 1])) {
                        if (tmp3.sum[i, 2] == sum3[length(sum3[, 
                          1]), 3]) {
                          sum3[length(sum3[, 1]), 3] <- tmp3.sum[i, 
                            3]
                        }
                        else {
                          sum3 <- rbind(sum3, tmp3.sum[i, ])
                        }
                      }
                    }
    ...
    for (u in facs) {
        ...
        if (length(sums[sums[, 1] == u, ]) == 0) {
            sums <- rbind(sums, cbind(u, JJu, JJu))
        }
        else {
            ...
            if (length(miss) > 0) {
                ...
                for (h in 1:length(miss)) {
                  if (max((miss[h] >= sums.u[, 2]) * (miss[h] <= 
                    sums.u[, 3])) == 0) {
                    miss.tmp <- c(miss.tmp, miss[h])
                  }
                }
    ...
    for (u in facs) {
        if (is.null(rems) == FALSE) {
            rem.col <- unique(as.vector(rems[rems[, 1] == u, 
                2:3]))
            rem.col <- sort(rem.col[is.na(rem.col) == FALSE])
            sums2 <- NULL
            for (o in 1:length(sums[, 1])) {
                if ((sums[o, 2] %in% rem.col) == FALSE) {
                  sums2 <- rbind(sums2, sums[o, ])
                }
            }
            sums <- sums2
        }
    ...
    for (s in 1:length(JJs)) {
        if (s %in% sums[, 1]) {
            ...
            for (d in 1:length(sums.s[, 1])) {
                Y.col <- cbind(Y.col, apply(matrix(Y[, sums.s[d, 
                  2]:sums.s[d, 3]], nrow = dim(Y)[1]), 1, sum))
            }
        }
        else {
