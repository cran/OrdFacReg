    ...
    for (s in 1:length(unique(JJs.A[, 2]))) {
        ...
        if (JJs.A.s[1, 5] == 1) {
            if (nrow(JJs.A.s) == 1) {
                ...
                n.act <- cbind(JJs.A.s[, 5], c(JJs.A.s[2:(nrow(JJs.A.s)), 
                  5], NA))
                n.act <- min(nrow(JJs.A.s), (1:nrow(JJs.A.s))[(n.act[, 
                  1] == n.act[, 2]) == 0], na.rm = TRUE)
            }
        ...
        if (length(JJs.A.s) > 0) {
            ...
            if (n.col > 0) {
                ...
                for (i in 1:n.col) {
                  beta <- c(beta, rep(beta.col.tmp[i], sums.tmp[i, 
                    4]))
                }
                ind <- ind + n.col
            }
